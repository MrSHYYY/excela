...
export default function SetupClient({ initialSession }: { initialSession: SignedInSession }) {
  ...
  return <div className={setup.shell}>
    <header className={setup.header}>
      ...
      <Link href="/" className={styles.textButton}>← Dashboard</Link>
    </header>
    <main className={setup.main}>
      ...
      <div className={setup.finish}><p>{complete ? "Your planner and API key are saved. You are ready." : "Connect a planner and save your API key to continue."}</p><button type="button" className={styles.primary} disabled={!complete || Boolean(busy)} onClick={() => { router.push("/"); router.refresh(); }}>Go to dashboard ↗</button></div>
    </main>
  </div>;
}
